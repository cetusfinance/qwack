...
{
    public class QPDoubleChoiceOption : IHasVega, IAssetInstrument
    {
        ...
        public DateTime DecisionDate1 { get; set; }
        public DateTime DecisionDate2 { get; set; }
        ...
        public double IntrinsicValue(IAssetFxModel model, DateTime valDate)
        {
            var curve = model.GetPriceCurve(AssetId);
            var settle = SettlementFixingDates.Average(curve.GetPriceForFixingDate);
            var periodAvgs = FixingDates.Select((d,ix)=> d.Average(curve.GetPriceForFixingDate)-(PeriodPremia?.Length>ix ? PeriodPremia[ix] : 0)).ToArray();

            if (CallPut == OptionType.C)
            {
                var minP = periodAvgs.Min();
                return settle - minP;
            }
            else if (CallPut == OptionType.P)
            {
                var maxP = periodAvgs.Max();
                return maxP - settle;
            }
            else
        ...
        public IAssetInstrument Clone() => new QPDoubleChoiceOption
        {
            TradeId = TradeId,
            Notional = Notional,
            Direction = Direction,
            PeriodDates = PeriodDates,
            DecisionDate1 = DecisionDate1,
            DecisionDate2 = DecisionDate2,
        ...
        {
            if(to_instrument.AssetInstrumentType != AssetInstrumentType.QPDoubleChoiceOption)
            {
                throw new InvalidOperationException("Incorrect instrument type");
            }
            var to = to_instrument.QPDoubleChoiceOption ?? throw new InvalidOperationException("Missing qp double choice option details");

            TradeId = to.TradeId;
            Notional = to.Notional;
            Direction = to.Direction;
            PeriodDates = to.PeriodDates;
            DecisionDate1 = to.DecisionDate1;
            DecisionDate2 = to.DecisionDate2;
        ...
        {
            AssetInstrumentType = AssetInstrumentType.QPDoubleChoiceOption,
            QPDoubleChoiceOption = new TO_QPDoubleChoiceOption
            {
                TradeId = TradeId,
                Notional = Notional,
                Direction = Direction,
