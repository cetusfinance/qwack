...
namespace Qwack.Math.Tests.Options.VolSurfaces
{
    public class GridVolSurfaceFacts
    {
        ...
        public void GridVolSurfaceAbsolute()
        {
            ...
            var vol = surface.GetVolForAbsoluteStrike(1.75, maturities[0].AddDays(15));
            var deltaK = BlackFunctions.BlackDelta(1.5, 1.75, 0, surface.ExpiriesDouble[0] + 15.0 / 365.0, vol, OptionType.C);
            var volForDeltaK = surface.GetVolForDeltaStrike(deltaK, maturities[0].AddDays(15));
        ...
        public void GridVolSurfaceDelta()
        {
            ...
            Assert.Equal(vols[0][0], surface.GetVolForAbsoluteStrike(3, origin.AddDays(33)), 12);
            Assert.Equal(vols[0][0], surface.GetVolForDeltaStrike(-0.3, origin.AddDays(303)), 12);
            Assert.Equal(vols[0][0], surface.GetVolForAbsoluteStrike(0.5, 0.777), 12);
            Assert.Equal(vols[0][0], surface.GetVolForDeltaStrike(0.9, 0.123), 12);
